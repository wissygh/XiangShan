...
class TraceIO(implicit val p: Parameters) extends Bundle with TraceConfig {
  ...
  val toPcMem        = Vec(GroupNum, ValidIO(new TraceBlock(false, IretireWidthCompressed)))
  val fromPcMem      = Input(Vec(GroupNum, UInt(IaddrWidth.W)))
