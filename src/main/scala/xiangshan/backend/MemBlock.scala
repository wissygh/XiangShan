  ...
  with SdtrigExt
{
  ...
  for (i <- 0 until LduCnt) {
    ...
    io.mem_to_ooo.writebackLda(i).bits.backendTrigger.backendHit := loadTriggerHitVec
    io.mem_to_ooo.writebackLda(i).bits.backendTrigger.backendCanFire := loadTriggerCanFireVec
  ...
  for (i <- 0 until HyuCnt) {
    ...
    io.mem_to_ooo.writebackHyuLda(i).bits.backendTrigger.backendHit := loadTriggerHitVec
    io.mem_to_ooo.writebackHyuLda(i).bits.backendTrigger.backendCanFire := loadTriggerCanFireVec
    ...
    when(hybridUnits(i).io.stout.fire &&
      ...
      hyuOut.bits.backendTrigger.backendHit := storeTriggerHitVec
      hyuOut.bits.backendTrigger.backendCanFire := storeTriggerCanFireVec
    }
  ...
  for (i <- 0 until StaCnt) {
    ...
    when(stOut(i).fire) {
      ...
      stOut(i).bits.backendTrigger.backendHit := storeTriggerHitVec
      stOut(i).bits.backendTrigger.backendCanFire := storeTriggerCanFireVec
    }
  }
  ...
  when (atomicsUnit.io.out.valid) {
    ...
    (0 until LduCnt).map(i => {
      io.mem_to_ooo.writebackLda(i).bits.backendTrigger.backendHit := VecInit(Seq.fill(TriggerNum)(false.B))
    })
    (0 until HyuCnt).map(i => {
      io.mem_to_ooo.writebackHyuLda(i).bits.backendTrigger.backendHit := VecInit(Seq.fill(TriggerNum)(false.B))
    })
  }
  ...
  io.mem_to_ooo.writeBack.map(wb => wb.bits.uop.trigger.clear())
